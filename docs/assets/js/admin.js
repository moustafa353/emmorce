...
if (typeof requireRole !== 'function') {
    console.error('requireRole function not found');
    ...
    orders.forEach(o => {
        ...
        row.innerHTML = `
            ...
            <div class="actions">
                ...
                <button class="delete-btn" data-order-id="${o.id}">Delete</button>
            </div>
        `;
...
async function deleteOrder(orderId) {
    ...
    try {
        ...
        const store = tx.objectStore('orders');
        const req = store.delete(parseInt(orderId));
        
        req.onsuccess = () => {
            renderOrders(); 
        };
        req.onerror = (e) => {
            console.error('Error deleting order:', e.target.error);
            alert('Error deleting order: ' + e.target.error);
        };
        
        tx.oncomplete = () => {}
        tx.onerror = (e) => console.error('Delete transaction error:', e.target.error);
    } catch (error) {
